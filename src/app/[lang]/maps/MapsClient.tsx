...
export default function MapsClient({ lang, maps, dictionary }: { lang: Locale, maps: Article[], dictionary: any }) {
    const locale = lang;
                                ...
                                {categoryMaps.map((map) => (
                                    ...
                                    >
                                        ...
                                        <div className="relative aspect-video w-full bg-zinc-800 border-b border-zinc-800 flex items-center justify-center overflow-hidden">
                                            {map.image ? (
                                                <img src={map.image} alt={map.title} className="object-cover w-full h-full opacity-80 group-hover:opacity-100 transition-opacity" />
                                            ) : (
                                        ...
                                        <div className="p-6 flex flex-col flex-1">
                                            ...
                                            <div className="flex items-center justify-between text-xs font-mono text-zinc-500 mb-2 tracking-wider">
                                                <span>{new Date(map.date).toLocaleDateString(locale === 'zh' ? 'zh-CN' : 'en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</span>
                                                {map.readTime && (
                                                    <span className="text-zinc-400">{map.readTime}</span>
                                                )}
                                            </div>
